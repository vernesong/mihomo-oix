...
func TestProxyDelayAcceptsTimeoutAboveInt16(t *testing.T) {
	groupName, _, _, delayServerURL := setupURLTestRoute(t)

	delayPath := "/" + url.PathEscape(groupName) + "/delay?url=" + url.QueryEscape(delayServerURL) + "&timeout=60000&expected=204"
	delayReq := httptest.NewRequest(http.MethodGet, delayPath, nil)
	delayRes := httptest.NewRecorder()
	proxyRouter().ServeHTTP(delayRes, delayReq)
	if delayRes.Code != http.StatusOK {
		t.Fatalf("GET proxy delay status = %d, body = %s", delayRes.Code, delayRes.Body.String())
	}
}

func TestProxyDelayUsesRequestContext(t *testing.T) {
	groupName, _, _, delayServerURL := setupURLTestRoute(t)

	delayPath := "/" + url.PathEscape(groupName) + "/delay?url=" + url.QueryEscape(delayServerURL) + "&timeout=1000&expected=204"
	delayReq := httptest.NewRequest(http.MethodGet, delayPath, nil)
	ctx, cancel := context.WithCancel(delayReq.Context())
	cancel()
	delayRes := httptest.NewRecorder()
	proxyRouter().ServeHTTP(delayRes, delayReq.WithContext(ctx))
	if delayRes.Code != http.StatusGatewayTimeout {
		t.Fatalf("GET canceled proxy delay status = %d, body = %s", delayRes.Code, delayRes.Body.String())
	}
}
