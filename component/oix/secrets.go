...
func decodeSecret(encoded string, legacySeed byte) string {
	if strings.HasPrefix(encoded, "v2:") {
		...
		recordSecretDecodeError(errors.New("invalid v2 secret"))
		return ""
	}
...
func decodeXORHex(encoded string, keySeed byte) string {
	...
	if err != nil {
		recordSecretDecodeError(fmt.Errorf("invalid xor hex: %w", err))
		return ""
	}
	for i := range data {
...
func recordSecretDecodeError(err error) {
	...
	secretDecodeErr = errors.Join(secretDecodeErr, err)
}
