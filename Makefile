...
ifeq ($(BRANCH),Alpha)
VERSION=alpha-oix-$(shell git rev-parse --short HEAD)
...
GOBUILD=CGO_ENABLED=0 go build -buildvcs=false -tags with_gvisor -trimpath -ldflags '-X "github.com/metacubex/mihomo/constant.Version=$(VERSION)" \
